...
(function () {
  ...
  style.textContent = `
    .foresight-chat-fab{position:fixed;right:28px;bottom:28px;z-index:9999;border:0;border-radius:999px;padding:14px 20px;background:linear-gradient(90deg,#587cff,#774cff);color:#fff;font:800 13px Inter,sans-serif;cursor:pointer;box-shadow:0 14px 38px rgba(91,91,255,.35)}
    ...
    .foresight-chat.open{display:flex}.fc-head{padding:18px;display:flex;align-items:center;gap:12px;border-bottom:1px solid #202a3d}.fc-avatar{width:38px;height:38px;border-radius:11px;display:grid;place-items:center;background:linear-gradient(135deg,#627cff,#754cff);font-weight:800}.fc-title{font-weight:800;font-size:14px}.fc-sub{color:#7186aa;font-size:9px;margin-top:3px}.fc-close{margin-left:auto;background:none;border:0;color:#8190aa;font-size:22px;cursor:pointer}.fc-messages{flex:1;overflow:auto;padding:16px;display:flex;flex-direction:column;gap:10px}.fc-msg{max-width:90%;padding:11px 13px;border-radius:14px;font-size:12px;line-height:1.55;white-space:pre-line}.fc-ai{align-self:flex-start;background:#1a2231;color:#b9c7de}.fc-user{align-self:flex-end;background:#314b9a;color:#fff}.fc-quick{padding:10px 14px;display:flex;gap:7px;overflow-x:auto;border-top:1px solid #202a3d}.fc-quick button{flex:0 0 auto;border:1px solid #293852;background:#121b2b;color:#9eb2d5;border-radius:999px;padding:7px 10px;font-size:9px;cursor:pointer}.fc-input{padding:12px;display:flex;gap:8px;border-top:1px solid #202a3d}.fc-input input{flex:1;min-width:0;border:1px solid #29344a;border-radius:12px;background:#0a0f19;color:#fff;padding:11px;outline:none}.fc-input button{border:0;border-radius:12px;padding:0 14px;background:#5f8cff;color:#fff;cursor:pointer;font-weight:800}
  ...
  chat.innerHTML = `
    <div class="fc-head"><div class="fc-avatar">F</div><div><div class="fc-title">Foresight AI</div><div class="fc-sub">SUPPLY INTELLIGENCE ASSISTANT</div></div><button class="fc-close" aria-label="Close">×</button></div>
  ...
  function answer(question) {
    ...
    if (q.includes("risk") || q.includes("why")) {
      ...
      return "Generate a forecast first so I can analyze the active SKU risk.";
    }
    ...
    if (q.includes("forecast") || q.includes("demand")) {
      return `Current forecast for ${d.sku}: ${d.demand ?? "—"} units.\n\nThe Demand Forecast chart contains the full upcoming weekly projection.`;
    }
    ...
    if (q.includes("hello") || q === "hi" || q.includes("hey")) {
      return `Hey 👋 I'm ready. Ask me about ${d.sku}'s forecast, inventory risk, ₹ impact, or reorder recommendation.`;
    }

    return "I can help with:\n\n• Forecast demand\n• Inventory risk\n• Reorder recommendations\n• ₹ sales at risk & overstock\n• Model validation\n• Recommended action\n\nTry: “What is the business impact?”";
  }
