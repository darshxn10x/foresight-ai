...
function renderForecastChart(forecastData) {
    ...
    forecastChart = new Chart(canvas, {
            ...
            datasets: [
                {
                    ...
                    borderColor: "#6e88bb",
                    backgroundColor: "rgba(110,136,187,.08)",
                    borderWidth: 2,
                    pointRadius: 3,
                    tension: .35,
                    fill: true
                },
                {
                    ...
                    borderColor: "#6e8cff",
                    backgroundColor: "rgba(110,140,255,.08)",
                    borderWidth: 3,
                    borderDash: [7, 6],
                    pointRadius: 4,
                    tension: .35,
        ...
        options: {
            ...
            plugins: {
                legend: {
                    labels: { color: "#899bb9", font: { size: 11 }, usePointStyle: true }
                }
            },
            scales: {
                x: { grid: { color: "rgba(120,140,180,.07)" }, ticks: { color: "#627493" } },
                y: {
                    beginAtZero: true,
                    grid: { color: "rgba(120,140,180,.07)" },
                    ticks: { color: "#627493" },
                    title: { display: true, text: "Units per Week", color: "#7184a4" }
                }
            }
        }
    });
...
async function initializeDashboard() {
    ...
    try {
        ...
        if ([currentStock, leadTimeDays, safetyStock].some(value => value < 0)) {
...
document.addEventListener("keydown", event => {
    if (event.key === "Enter" && !event.target.matches("input")) initializeDashboard();
});
